...
| 4 | Adapter Studio | Runtime catalog, connector certification, fixture replay, Integration Runtime, standalone replay API, and generated SDK operation plans show how an external provider is connected without exposing secrets. | `adapterStudio`, `connectorFixtureReplay`, `integrationRuntime`, `GET /demo/connector-fixture-replay`, `GET /demo/integration-runtime`, `CONNECTOR_CERTIFICATION.md`, `CONNECTOR_FIXTURE_REPLAY.md`, `INTEGRATION_RUNTIME.md`, `adapter_runtime.previewed`, `sdk/generated/public-demo/` |
| 5 | Async execution | Accepted work moves through outbox and worker-backed contracts with retry, idempotency, reconciliation, and incident-route boundaries. | `outbox`, `integrationJobs`, `integrationRuntime`, `worker:drivedesk_worker.main.process_pending_outbox`, `adapter_runtime.previewed` |
...
4. `Integrations` - Adapter Studio, connector certification, fixture replay,
   Integration Runtime, adapter contracts, operation scenarios, sync jobs, and
   health.
