...
| Business control tower | Cross-system observations, business exception queue, approval-gated repair action, dry-run execution, aggregate metrics, and public demo payload are covered. | `docs/public/BUSINESS_CONTROL_TOWER.md`, `apps/admin/public-demo/index.html`, `GET /demo/public` | `bash scripts/check_public_business_control_tower.sh && bash scripts/check_public_demo_api.sh` |
