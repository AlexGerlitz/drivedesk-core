...
| Generated client SDK | Python, JavaScript, and TypeScript demo clients plus typed file-import, CRM provider-intake, and worker-backed adapter operation plans with Adapter Studio validation | `docs/public/CLIENT_SDK.md`, `docs/public/ADAPTER_DEVELOPER_GUIDE.md`, `sdk/generated/public-demo/`, `examples/python/demo_adapter_operation_plan.py`, `examples/js/demo-adapter-operation-plan.mjs` | `bash scripts/check_public_demo_sdk.sh && bash scripts/check_public_adapter_developer_guide.sh` |
...
| Integration adapter model | Runtime adapter catalog, public-safe provider connector guide, Adapter Studio, adapter developer guide, CRM deal intake adapter, auth profiles, secret boundaries, operation scenarios, typed SDK operation plans, mapping preview, scopes, diagnostics, reconciliation, incident cards | `apps/admin/public-demo/index.html`, `docs/public/PROVIDER_CONNECTOR_GUIDE.md`, `docs/public/ADAPTER_DEVELOPER_GUIDE.md`, `docs/public/INTEGRATION_ADAPTER_CATALOG.md`, `docs/public/INTEGRATION_OPERATION_CONTRACTS.md`, `docs/public/INTEGRATION_RECONCILIATION.md`, `docs/public/CLIENT_SDK.md`, `docs/adr/0072-mock-crm-deal-adapter.md`, `docs/adr/0073-adapter-auth-profile-boundary.md` | `bash scripts/check_public_provider_connector_guide.sh && bash scripts/check_public_adapter_developer_guide.sh && bash scripts/check_public_demo_api.sh && bash scripts/check_public_demo_sdk.sh` |
...
7. Run `bash scripts/ci_smoke_public.sh`.
8. Run `bash scripts/check_public_system_review_path.sh`.
9. Run `bash scripts/check_public_reviewer_quickstart.sh`.
10. Run `bash scripts/check_public_project_status.sh`.
11. Run `bash scripts/check_public_technical_capability_map.sh`.
12. Run `bash scripts/check_public_evidence_index.sh`.
13. Run `bash scripts/check_public_business_control_tower.sh`.
14. Run `bash scripts/check_public_observability_proof.sh`.
15. Run `bash scripts/check_public_alert_routing.sh`.
16. Run the capability-specific verifier from the table above.
